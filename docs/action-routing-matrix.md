...
| `/dashboard/main` | View calendar | Routed to dashboard operations calendar | Open scheduled live calendar | `/live/calendar`, `GET /api/calendar-events` | `main_admin` | Implemented | `backend.test.ts` assistant/quick action route guard, `check:links`, build |
